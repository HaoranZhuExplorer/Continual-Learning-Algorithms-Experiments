...
def train_net_ewc(optimizer, ewc, criterion, mb_size, x, y, t,
                  train_ep, preproc=None, use_cuda=True):
    """
    (EWC Version)
    ...
    for ep in range(train_ep):
        ...
        for it in range(it_x_ep):
            ...
            loss += ewc._compute_consolidation_loss(ewc.weight)
            ...
            if it % 100 == 0:
                ...
                # print("cuda memory: "+str(cutorch.memory_summary()))
            
        cur_ep += 1

    return ave_loss, acc, stats, TensorDataset(maybe_cuda(train_x, use_cuda=use_cuda),
                                               maybe_cuda(train_y, use_cuda=use_cuda))
